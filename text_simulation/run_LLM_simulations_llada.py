"""
Run LLM Simulations using LLaDA 2.0 Model

This script is adapted from run_LLM_simulations_dream.py to use LLaDA 2.0 instead of
Dream or OpenAI/Gemini API models. LLaDA 2.0 is a local masked diffusion language model.

Usage:
    python run_LLM_simulations_llada.py --config text_simulation/configs/llada_config.yaml --max_personas 5
"""
...
from llm_helper_llada import LLaDALLMConfig, process_prompts_batch_llada
...
def save_and_verify_callback(prompt_id: str, llm_response_data: dict, original_prompt_text: str, **kwargs) -> bool:
    """
    Saves the LLaDA LLM response and then verifies it.
    ...
    output_json_data = {
        ...
        "model": "LLaDA-2.0"  # Mark as LLaDA model
    }
    try:
...
async def run_simulations(
    ...
    """
    Run LLaDA 2.0 simulations for all prompts.
    
    Args:
        prompts_root_dir: Directory containing prompt files
        base_output_dir: Directory to save outputs
        llada_config_params: Dictionary of LLaDA configuration parameters
    ...
    llada_config = LLaDALLMConfig(
        model_name=llada_config_params.get('model_name', 'ML-GSAI/LLaDA-8B-Instruct'),
        temperature=llada_config_params.get('temperature', 0.2),
        max_new_tokens=llada_config_params.get('max_new_tokens', 512),
        steps=llada_config_params.get('steps', 512),
        remasking_strategy=llada_config_params.get('remasking_strategy', 'random'),
        guidance_scale=llada_config_params.get('guidance_scale', 1.0),
        top_p=llada_config_params.get('top_p', 0.95),
        top_k=llada_config_params.get('top_k', None),
        system_instruction=llada_config_params.get('system_instruction'),
        max_retries=max_retries_for_sequence,
        max_concurrent_requests=num_workers,
        device=llada_config_params.get('device', 'cuda'),
        verification_callback=save_and_verify_callback,
        verification_callback_args=verification_args,
        max_context_length=llada_config_params.get('max_context_length', 2048)
    )
    ...
    if not prompts_to_process:
        print("No new files require LLaDA processing or re-processing.")
        if skipped_due_to_existing_verified_count == 0:
            print("No prompt files found or processed in this run.")
        return

    print(f"Found {len(prompts_to_process)} prompts to process with LLaDA 2.0 (up to {llada_config.max_concurrent_requests} concurrent requests).")
    print(f"Note: LLaDA is a local model. Processing may be slower than API-based models.")

    # Process prompts with LLaDA
    final_results = await process_prompts_batch_llada(
        prompts_to_process,
        llada_config,
        desc="LLaDA 2.0 calls & Verification"
    )
    ...
    print(f"\nLLaDA 2.0 processing run complete.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run LLaDA 2.0 simulations with integrated verification and retries.")
    ...
    base_output_dir = config_values.get('output_folder_dir', './text_simulation_output_llada')
    ...
    # Get LLaDA-specific config
    llada_config_dict = config_values.get('llada_config', {})
    if not llada_config_dict:
        # Fallback to top-level config
        llada_config_dict = {
            'model_name': config_values.get('model_name', 'ML-GSAI/LLaDA-8B-Instruct'),
            ...
            'remasking_strategy': config_values.get('remasking_strategy', 'random'),
            'guidance_scale': config_values.get('guidance_scale', 1.0),
    ...
    print(f"Starting LLaDA 2.0 simulations from: {args.config} at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Model: {llada_config_dict.get('model_name', 'ML-GSAI/LLaDA-8B-Instruct')}")
    ...
    asyncio.run(run_simulations(
        ...
        base_output_dir=base_output_dir,
        llada_config_params=llada_config_dict,
